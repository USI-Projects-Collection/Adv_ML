"""
Figure 8 (bottom) — variante "independent" del metodo Jiang.

Stesso protocollo di `run_figure8_bottom_ttreg.py` ma usa
`TTRegDINOv2Independent`: per ogni register neuron, prendiamo le top-N
posizioni patch più attive e copiamo i loro valori signed sui rispettivi
N TT register tokens (TT_REG_0 ← top-1, TT_REG_1 ← top-2, ecc.). Le top-N
posizioni patch vengono azzerate.

Output:
  results/figure8_bottom_ttreg_indep.json
  results/figure8_bottom_ttreg_indep.png
"""
import json
import sys
import time
...
OUT_FILE = ROOT / "results" / "figure8_bottom_ttreg_indep.json"
OUT_PNG = ROOT / "results" / "figure8_bottom_ttreg_indep.png"
...
def main():
    ...
    for n in REGS:
        ...
        t0 = time.time()
        try:
            m = load_dinov2_with_tt_registers_independent(n, neurons, img_size=IMG_SIZE)
    ...
    axes[0].set_title("ImageNet (tt-reg independent)")
    ...
    axes[1].set_title("ADE20k segmentation (tt-reg indep)")
    ...
    axes[2].set_title("NYUd depth (tt-reg indep)")
